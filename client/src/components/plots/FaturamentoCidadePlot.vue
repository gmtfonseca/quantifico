    ...
    <template v-slot:filters>
      <div class="filters">
        ...
        <QSlider
          ...
          label="Limite"
          hint="Quantas cidades deseja visualizar?"
          :min="1"
          :max="15"
        />
      </div>
...
class FaturamentoCidadePlot extends Plot {
  async getSeries() {
    const httpService = new HttpService('nfs/plot/faturamento-cidade')
    const response = await httpService.get(this._params)
    const data = response.data.map(record => {
      return {
        x: record.descricaoMunicipio,
...
function initialState() {
  return {
    plot: new FaturamentoCidadePlot(),
    filters: {
      startDate: null,
      endDate: null,
      limit: 10,
...
export default {
  ...
  sockets: {
    ...
    refresh() {
      console.log('refresh CityRevenuePlot')
      this.build()
    }
  },
  mounted() {
